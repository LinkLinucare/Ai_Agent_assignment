...
# Here we define the state model
# List all candidates, their scores, merges the data and stores the feedback for human review
...
# Main workflow
# Sets the state as empty first
class LeadScoreFlow(Flow[LeadScoreState]):
    initial_state = LeadScoreState
# begin loeading leads from the csv fiel
    @start()
    def load_leads(self):
        import csv
...
# Send the candidate profile for review
...
def run_crew():
    """Runs the LeadScoreFlow CrewAI workflow."""
    kickoff()  # Call the existing kickoff() function